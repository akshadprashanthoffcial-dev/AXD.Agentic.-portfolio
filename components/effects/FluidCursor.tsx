 ...
 * background behind all content. Splat colors are biased to the axd.labs
 * palette (orange / purple / red). Adapted from the classic GPU fluid
 * solver (Pavel Dobryakov), trimmed and re-colored.
 *
 * Listens on window (canvas is pointer-events:none) so it never blocks UI.
...
export default function FluidCursor() {
  ...
  useEffect(() => {
    ...
    // Brand palette (0..1 rgb), pre-scaled down for dye injection.
    const BRAND = [
      { r: 0.96, g: 0.61, b: 0.0 }, // #F59B00
      { r: 0.63, g: 0.12, b: 0.95 }, // #A11FF2
      { r: 0.94, g: 0.24, b: 0.25 }, // #EF3C3F
    ];
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      if (isWebGL2) {
        ...
        supportLinearFiltering = (gl as any).getExtension(
    ...
    const displayMaterial = new Program(
      baseVertexShader,
    ...
    const onMove = (e: MouseEvent) => {
      ...
      const posX = (e.clientX - rect.left) * (window.devicePixelRatio || 1);
      const posY = (e.clientY - rect.top) * (window.devicePixelRatio || 1);
      const color = pointer.moved ? pointer.color : brandColor();
    ...
    const onTouch = (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        const posX = (touches[i].clientX - rect.left) * (window.devicePixelRatio || 1);
        const posY = (touches[i].clientY - rect.top) * (window.devicePixelRatio || 1);
        updatePointerMoveData(pointer, posX, posY, brandColor());
      }
    };
